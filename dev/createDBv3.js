...
// - v3 adds state tables for stalls + rollup/summary tables + refined alert uniqueness
// - v3.1: summary_windows uses composite PK and split admin flags + actual-end audit fields
...
try {
  ...
  db.exec(`CREATE TABLE IF NOT EXISTS datasources ( datasource_name TEXT PRIMARY KEY )${STRICT};`);
  ...
  // ========= oracle_stall_state =========
  db.exec(`
    CREATE TABLE IF NOT EXISTS oracle_stall_state (
      chain_id          INTEGER NOT NULL,
      contract_address  TEXT NOT NULL CHECK (contract_address  = LOWER(contract_address)),
      validator_address TEXT NOT NULL CHECK (validator_address = LOWER(validator_address)),
      status            TEXT NOT NULL DEFAULT 'ok' CHECK (status IN ('ok','candidate','stalled')),
      consec_bad        INTEGER NOT NULL DEFAULT 0,
      consec_good       INTEGER NOT NULL DEFAULT 0,
      first_bad_run_id  INTEGER,
      last_seen_run_id  INTEGER,
      updated_at        DATETIME DEFAULT CURRENT_TIMESTAMP,
      PRIMARY KEY (chain_id, contract_address, validator_address),
      FOREIGN KEY (chain_id, contract_address)  REFERENCES contracts(chain_id, address)  ON DELETE CASCADE,
      FOREIGN KEY (chain_id, validator_address) REFERENCES validators(chain_id, address) ON DELETE CASCADE
    )${STRICT};
  `);
  ...
  db.exec(`
    DROP TRIGGER IF EXISTS trg_oracle_stall_state_updated_at;
    CREATE TRIGGER trg_oracle_stall_state_updated_at
    AFTER UPDATE ON oracle_stall_state
    FOR EACH ROW
    WHEN NEW.updated_at <= OLD.updated_at
    BEGIN
      ...
      WHERE chain_id = NEW.chain_id
        ...
        AND validator_address = NEW.validator_address;
    END;
  `);

  // ========= summary_windows (ledger to avoid duplicate DMs) =========
  // v3.1: composite PK + split admin flags + "actual end" audit fields
  db.exec(`
CREATE TABLE IF NOT EXISTS summary_windows (
  window_start        DATETIME NOT NULL,
  window_end          DATETIME NOT NULL,
  owners_done         INTEGER NOT NULL DEFAULT 0 CHECK (owners_done IN (0,1)),
  admins_oracle_done  INTEGER NOT NULL DEFAULT 0 CHECK (admins_oracle_done IN (0,1)),
  admins_ds_done      INTEGER NOT NULL DEFAULT 0 CHECK (admins_ds_done IN (0,1)),
  oracle_actual_start DATETIME,   -- ✅ add
  oracle_actual_end   DATETIME,
  ds_actual_start     DATETIME,   -- ✅ add
  ds_actual_end       DATETIME,
  ...
  `);
  db.exec(`
    CREATE INDEX IF NOT EXISTS idx_summary_windows_processed_at
    ON summary_windows(processed_at);
  `);

  // ========= oracle_health_rollup (per window per validator@contract) =========
  db.exec(`
    CREATE TABLE IF NOT EXISTS oracle_health_rollup (
      window_start      DATETIME NOT NULL,
      window_end        DATETIME NOT NULL,
      chain_id          INTEGER NOT NULL,
      contract_address  TEXT    NOT NULL CHECK (contract_address  = LOWER(contract_address)),
      ...
      stalled_hits      INTEGER NOT NULL DEFAULT 0,
      -- useful “last” metrics for human-friendly DMs
      last_dev_pct      REAL,
      last_span_sec     INTEGER,
      last_median_now   REAL,
      last_price        REAL,
      open_at_end       INTEGER NOT NULL DEFAULT 0 CHECK (open_at_end IN (0,1)),
      first_seen_run_id INTEGER,
      last_seen_run_id  INTEGER,
      updated_at        DATETIME DEFAULT CURRENT_TIMESTAMP,
      PRIMARY KEY (window_start, chain_id, contract_address, validator_address),
      FOREIGN KEY (chain_id, contract_address)  REFERENCES contracts(chain_id, address)  ON DELETE CASCADE,
      FOREIGN KEY (chain_id, validator_address) REFERENCES validators(chain_id, address) ON DELETE CASCADE
    )${STRICT};
  `);
  ...
  db.exec(`
    DROP TRIGGER IF EXISTS trg_oracle_health_rollup_updated_at;
    CREATE TRIGGER trg_oracle_health_rollup_updated_at
    AFTER UPDATE ON oracle_health_rollup
    FOR EACH ROW
    WHEN NEW.updated_at <= OLD.updated_at
    BEGIN
      UPDATE oracle_health_rollup
      SET updated_at = CURRENT_TIMESTAMP
  ...
  // ========= datasource_health_rollup (per window per datasource@contract) =========
  db.exec(`
    CREATE TABLE IF NOT EXISTS datasource_health_rollup (
      window_start      DATETIME NOT NULL,
      window_end        DATETIME NOT NULL,
      chain_id          INTEGER NOT NULL,
      contract_address  TEXT    NOT NULL CHECK (contract_address  = LOWER(contract_address)),
      ...
      ok_hits           INTEGER NOT NULL DEFAULT 0,
      stalled_hits      INTEGER NOT NULL DEFAULT 0,
      outlier_hits      INTEGER NOT NULL DEFAULT 0,
      fetch_error_hits  INTEGER NOT NULL DEFAULT 0,
      last_dev_pct      REAL,
      last_span_sec     INTEGER,
      last_median_now   REAL,
      last_price        REAL,
      ...
      last_seen_run_id  INTEGER,
      updated_at        DATETIME DEFAULT CURRENT_TIMESTAMP,
      PRIMARY KEY (window_start, chain_id, contract_address, datasource_name),
      FOREIGN KEY (chain_id, contract_address) REFERENCES contracts(chain_id, address) ON DELETE CASCADE,
      FOREIGN KEY (datasource_name)            REFERENCES datasources(datasource_name) ON DELETE CASCADE
    )${STRICT};
  `);
  ...
  db.exec(`
    DROP TRIGGER IF EXISTS trg_datasource_health_rollup_updated_at;
    CREATE TRIGGER trg_datasource_health_rollup_updated_at
    AFTER UPDATE ON datasource_health_rollup
    FOR EACH ROW
    WHEN NEW.updated_at <= OLD.updated_at
    BEGIN
      UPDATE datasource_health_rollup
      SET updated_at = CURRENT_TIMESTAMP
      WHERE window_start     = NEW.window_start
        AND chain_id         = NEW.chain_id
        AND contract_address = NEW.contract_address
        AND datasource_name  = NEW.datasource_name;
    END;
  `);

  // Helpful rollup indexes
  db.exec(`CREATE INDEX IF NOT EXISTS idx_oracle_rollup_window ON oracle_health_rollup(window_start, window_end);`);
  db.exec(`CREATE INDEX IF NOT EXISTS idx_ds_rollup_window     ON datasource_health_rollup(window_start, window_end);`);

  // ========= datasource_stall_state =========
  db.exec(`
    CREATE TABLE IF NOT EXISTS datasource_stall_state (
      chain_id          INTEGER NOT NULL,
      contract_address  TEXT    NOT NULL CHECK (contract_address = LOWER(contract_address)),
      datasource_name   TEXT    NOT NULL,
      first_seen_run_id INTEGER,
      ...
      last_seen_run_id  INTEGER,
      consec_stalls     INTEGER NOT NULL DEFAULT 0,
      consec_ok         INTEGER NOT NULL DEFAULT 0,
      is_open           INTEGER NOT NULL DEFAULT 0 CHECK (is_open IN (0,1)),
      updated_at        DATETIME DEFAULT CURRENT_TIMESTAMP,
      PRIMARY KEY (chain_id, contract_address, datasource_name),
      FOREIGN KEY (chain_id, contract_address) REFERENCES contracts(chain_id, address) ON DELETE CASCADE,
      FOREIGN KEY (datasource_name)            REFERENCES datasources(datasource_name) ON DELETE CASCADE
    )${STRICT};
  `);
  ...
  db.exec(`
    DROP TRIGGER IF EXISTS trg_datasource_stall_state_updated_at;
    CREATE TRIGGER trg_datasource_stall_state_updated_at
    AFTER UPDATE ON datasource_stall_state
    FOR EACH ROW
    WHEN NEW.updated_at <= OLD.updated_at
    BEGIN
  ...
  // Alerts (refined uniqueness: three disjoint scopes)
  db.exec(`
    CREATE UNIQUE INDEX IF NOT EXISTS ux_alert_open_contract_only
    ON alerts(discord_id, chain_id, contract_address, alert_type)
    WHERE resolved_at IS NULL
      AND contract_address IS NOT NULL
      AND validator_address IS NULL;
  `);
  db.exec(`
    CREATE UNIQUE INDEX IF NOT EXISTS ux_alert_open_validator_only
  ...
  `);
  db.exec(`
    CREATE UNIQUE INDEX IF NOT EXISTS ux_alert_open_validator_contract
    ON alerts(discord_id, chain_id, validator_address, contract_address, alert_type)
    WHERE resolved_at IS NULL
      AND validator_address IS NOT NULL
      AND contract_address IS NOT NULL;
  `);
  ...
  // Datasource stall quick lookup
  db.exec(`
    CREATE INDEX IF NOT EXISTS idx_ds_stall_open ON datasource_stall_state(is_open) WHERE is_open = 1;
  `);
