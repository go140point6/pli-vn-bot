...
// - Option C: explicit ingest runs (ingest_runs + run_id in datasource_price_snapshots)

// dev/createDB.js
const { openDb, dbFile } = require('../db');   // ← single import path
const Database = require('better-sqlite3');    // (optional, openDb already constructs)
const db = openDb({ fileMustExist: false });   // ← allows creating the file
...
`);
  ...
  // ========= validators (multi-chain) =========
  // address is canonical lowercase 0x..., with eip55 (display), addr_format ('0x'|'xdc') for UX
  db.exec(`
    CREATE TABLE IF NOT EXISTS validators (
      ...
      address_eip55 TEXT,                                 -- presentation (computed in app)
      addr_format   TEXT NOT NULL DEFAULT '0x'            -- '0x' or 'xdc' (for display preference)
        CHECK (addr_format IN ('0x','xdc')),
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS contracts (
      ...
      address  TEXT NOT NULL
        ...
        CHECK (LOWER(address) LIKE '0x%' AND LENGTH(address) = 42),
      address_eip55 TEXT,
      addr_format   TEXT NOT NULL DEFAULT '0x'
        CHECK (addr_format IN ('0x','xdc')),
      pair  TEXT,
      base  TEXT NOT NULL,
      quote TEXT NOT NULL,
  ...
  // ========= validator_contracts (multi-chain) =========
  db.exec(`
  ...
  // ========= datasource_price_snapshots (multi-chain, per-run) =========
  db.exec(`
  ...
  // ========= oracle_price_snapshots (multi-chain, per-run) =========
  db.exec(`
  ...
  // ========= datasource_contract_map (multi-chain) =========
  db.exec(`
  ...
  // ========= alerts (multi-chain aware) =========
  // Note: Allow any non-empty alert_type so we can support DS_FETCH_ERROR:<source>, etc.
  db.exec(`
    CREATE TABLE IF NOT EXISTS alerts (
      id INTEGER PRIMARY KEY,
      discord_id TEXT NOT NULL,
      chain_id INTEGER,                          -- NULL for global alerts
      contract_address  TEXT,                    -- nullable
      validator_address TEXT,                    -- nullable
      alert_type TEXT NOT NULL,                  -- free-form tag (e.g., BALANCE_WARNING, DS_FETCH_ERROR:bitmart)
  ...
  // ========= price_aggregates (multi-chain) =========
  // Optional run_id for traceability back to the ingest run (nullable to support ad-hoc windows)
  db.exec(`
  ...
  // Fast lookup for all admins
  db.exec(`CREATE INDEX IF NOT EXISTS idx_users_is_admin ON users(is_admin) WHERE is_admin = 1;`);

  // Validators lookups
  db.exec(`CREATE INDEX IF NOT EXISTS idx_validators_discord ON validators(discord_id);`);

  // Contracts (active subset)
  db.exec(`
  ...
  // Snapshots (by contract/time and by run)
  db.exec(`
  ...
  // Alerts uniqueness (multi-chain aware)
  db.exec(`
  ...
  `);
  db.exec(`
    CREATE INDEX IF NOT EXISTS idx_price_aggregates_time
    ON price_aggregates(chain_id, window_end DESC);
  `);
