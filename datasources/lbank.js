...
async function fetchLBankPrices(client, runId, digest) {
  ...
  try {
    const apiMeta = mustGetApiMeta(datasource); // throws if not present
    const rows = selMappingsBySource.all(datasource);

    if (!Array.isArray(rows) || rows.length === 0) {
      console.warn('⚠️ No mappings for lbank.');
      return;
    }

    // LBank uses datasource_pair_id
    ...
    // If you removed LBank from datasources/datasource_apis during dev,
    // mustGetApiMeta() will throw; that’s fine — we log and exit gracefully.
